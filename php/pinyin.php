...
"|bin|bing|bo|bu|ca|cai|can|cang|cao|ce|ceng|cha|chai|chan|chang" .
"|chao|che|chen|cheng|chi|chong|chou|chu|chuai|chuan|chuang|chui" .
...
"|juan|jue|jun|ka|kai|kan|kang|kao|ke|ken|keng|kong|kou|ku|kua" .
"|kuai|kuan|kuang|kui|kun|kuo|la|lai|lan|lang|lao|le|lei|leng|li" .
"|lia|lian|liang|liao|lie|lin|ling|liu|long|lou|lu|lv|luan|lue" .
"|lun|luo|ma|mai|man|mang|mao|me|mei|men|meng|mi|mian|miao|mie" .
"|min|ming|miu|mo|mou|mu|na|nai|nan|nang|nao|ne|nei|nen|neng|ni" .
"|nian|niang|niao|nie|nin|ning|niu|nong|nu|nv|nuan|nue|nuo|o|ou" .
"|pa|pai|pan|pang|pao|pei|pen|peng|pi|pian|piao|pie|pin|ping|po" .
"|pu|qi|qia|qian|qiang|qiao|qie|qin|qing|qiong|qiu|qu|quan|que" .
"|qun|ran|rang|rao|re|ren|reng|ri|rong|rou|ru|ruan|rui|run|ruo" .
...
"|si|song|sou|su|suan|sui|sun|suo|ta|tai|tan|tang|tao|te|teng|ti" .
...
"|xiu|xu|xuan|xue|xun|ya|yan|yang|yao|ye|yi|yin|ying|yo|yong|you" .
"|yu|yuan|yue|yun|za|zai|zan|zang|zao|ze|zei|zen|zeng|zha|zhai" .
"|zhan|zhang|zhao|zhe|zhen|zheng|zhi|zhong|zhou|zhu|zhua|zhuai" .
...
$_pinyin = explode('|', $_pinyin);
$_code_low = explode('|', $_code_low);
$_encoding_low = array_combine($_code_low, $_pinyin);

if (!$_encoding_low)
{
	echo "Wrong encoding data!\n";
	die;
}

sort($_code_low);

function hanzi_to_pinyin($code)
{
	global $_pinyin;
	global $_code_low;
	global $_encoding_low;

	$r = ' ';

	for ($i = 0; $i < count($_code_low) - 1; $i++)
	{
		if (intval($code) >= intval($_code_low[$i]) 
			&& intval($code) < intval($_code_low[$i+1]))
		{
			$r =  $_encoding_low[$_code_low[$i]];
		}
	}
...
{
	$w = mb_substr($words, $i, 1);

	$gb = mb_convert_encoding($w, "gb2312");

	if (strlen($gb) == 1)
	{
		echo strtoupper($gb[0]);
	}
	else
	{
		$code = -(65536 - (ord($gb[0]) * 256 + ord($gb[1])));
//		echo "$code\n";
		echo hanzi_to_pinyin($code);
	}
